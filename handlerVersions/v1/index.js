...
function updateTransferData(state, payload, blockInfo, context) {
  ...
  console.log('ipfs hash:', payload.data.ipfs_url)

  var ipfs = IpfsApi({host: '127.0.0.1', port: '5001', protocol: 'http'})

  ipfs.pin.add(payload.data.ipfs_url, function (err, file) {
    if (err) {
      throw err
    }
...
function delDoc(state, payload, blockInfo, context) {
  console.log('updateTransferData')
  console.log('\n\nstate:\n', state, '\n\npayload:\n', payload, '\n\nblockInfo:\n', blockInfo, '\n\ncontext:\n', context)
  ...
  ipfs.pin.rm(payload.data.ipfs_url, function (err, file) {
    if (err) {
      throw err
    }
...
const updaters = [
  {
    actionType: "ipfs.pay::savedoc",
    apply: updateTransferData,
  },
  {
    actionType: "ipfs.pay::deldoc",
    apply: delDoc,
  },
]
